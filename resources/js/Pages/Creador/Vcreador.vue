<template>
  <Head title="Galería de Videos Creador" />
  ...
  <div class="fixed inset-0 flex flex-col bg-gray-50 text-gray-800 font-sans overflow-hidden">
    ...
    <div class="flex flex-1 flex-col md:flex-row overflow-hidden">
      ...
      <main class="flex-1 px-6 py-4 bg-gray-50 text-gray-800 overflow-y-auto">
        <div class="max-w-7xl mx-auto space-y-6">
            ...
            >
              ...
              <div class="flex-1 flex flex-col justify-between py-2 w-full space-y-4">
                ...
                <div class="flex items-center justify-between border-b border-gray-200 pb-4">
                  ...
                  <button class="bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded-xl text-xs transition-colors shadow-sm">
                    Gestionar
                  </button>
                </div>
          ...
          <!-- Empty State -->
          <div v-else class="text-center py-16 bg-white border border-gray-200 rounded-3xl text-gray-500 shadow-sm">
            <span class="text-6xl block mb-4">🎥</span>
            <h3 class="text-xl font-bold text-gray-900">No hay videos subidos</h3>
            <p class="text-sm text-gray-500 max-w-md mx-auto mt-2">Aún no se ha subido ningún video. ¡Haz clic en "+ Crear Video" para publicar el primero!</p>
          </div>
...
onMounted(() => {
  // Inicializar con el primer video si existe
  if (props.videos && props.videos.length > 0) {
    activeVideoId.value = props.videos[0].id;
  }

  observer = new IntersectionObserver((entries) => {
    // Buscar la entrada que es visible en el viewport
    ...
    root: null,
    threshold: 0.6 // Activar cuando el 60% del elemento sea visible en la pantalla
  });

  // Empezar a observar todos los mockups
...
const onIframeLoad = (event, videoType) => {
  ...
  const sendUnmute = () => {
    try {
      // Mandar comando de desmutear y play al reproductor oficial de TikTok
      ...
      // Ignorar posibles errores cross-origin de acceso denegado
    }
  };

  // Ejecutar en intervalos cortos para asegurar la inicialización del player de TikTok
...
const getPosterStyle = (url) => {
  ...
  // Gradiente premium y moderno para TikTok/genéricos
  return {
